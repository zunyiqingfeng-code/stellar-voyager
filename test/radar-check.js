...
const PORT = 9485;
const profile = path.join(require('os').tmpdir(), 'sv-radar-' + Date.now());
...
(async () => {
  ...
  try {
    ...
    await sleep(2000);
    // 雷达画布像素统计：非透明像素数应 > 500（环+扫描线+点）
    console.log('RADAR:', await ev('(function(){ var cv = document.getElementById("radar"); var cx = cv.getContext("2d"); var img = cx.getImageData(0, 0, cv.width, cv.height).data; var n = 0; for (var i = 3; i < img.length; i += 16) if (img[i] > 30) n++; return "非空采样=" + n + " 可见=" + (document.getElementById("radar").offsetParent !== null || document.getElementById("radar").style.display !== "none") + " 尺寸=" + cv.offsetWidth + "x" + cv.offsetHeight; })()'));
    console.log('LOOP:', await ev('JSON.stringify((window.__loopErrs || []).slice(0, 2))'));
    ws.close();
