...
(function (root) {
  ...
  // 每种行星类型的配色
  const PAL = {
    gaia:        { land: ['#2e7d3a', '#3f9b4f', '#6db86f'], sea: ['#1e4f8f', '#2f74c4', '#4fa0e8'], ice: '#eef6ff', cloud: 'rgba(255,255,255,0.78)' },
    continental: { land: ['#5e7a36', '#7a934a', '#9ab262'], sea: ['#14406e', '#1e5fa8', '#3a86d8'], ice: '#f2f8ff', cloud: 'rgba(255,255,255,0.75)' },
    ocean:       { land: ['#3a6a4a', '#4a7d5a'], sea: ['#0e3066', '#144080', '#2a5fa8'], ice: '#f0f8ff', cloud: 'rgba(255,255,255,0.72)' },
    tundra:      { land: ['#7d8a6a', '#93a07d', '#b8c0a0'], sea: ['#2a4a6a', '#3a6488'], ice: '#ffffff', cloud: 'rgba(255,255,255,0.65)' },
    ice:        { land: ['#c8dcef', '#dceaf7', '#b0c8e0'], sea: ['#7fa8cc', '#9cc0e0'], ice: '#ffffff', cloud: 'rgba(255,255,255,0.55)' },
    desert:      { land: ['#b98a4a', '#d2a45e', '#e8c37e'], sea: ['#a06a2a'], ice: '#f8f0e0', cloud: 'rgba(255,240,220,0.45)' },
    arid:        { land: ['#a8783c', '#c09050', '#d8b06a'], sea: ['#8a6028'], ice: '#f0e8d8', cloud: 'rgba(255,244,224,0.45)' },
    barren:      { land: ['#8a8478', '#9a948a', '#6e685e'], sea: ['#7a746a'], ice: '#cfc8bc', cloud: 'rgba(255,255,255,0.28)' },
    lava:        { land: ['#3a2018', '#2a1610', '#1c0f0a'], sea: ['#ff5a1e', '#ff8a2a', '#ffc04a'], ice: 'rgba(0,0,0,0)', cloud: 'rgba(0,0,0,0)' },
    toxic:       { land: ['#5a7a2a', '#6a8a3a', '#7a9a4a'], sea: ['#8aa050', '#a0b060'], ice: '#d0d8a0', cloud: 'rgba(220,255,140,0.55)' },
    tomb:        { land: ['#4a4a50', '#58585e', '#3c3c42'], sea: ['#2a2a30'], ice: '#8a8a90', cloud: 'rgba(160,160,170,0.35)' },
    machine:     { land: ['#3a3f46', '#4a5058', '#545c66'], sea: ['#2a2f36'], ice: '#7a828c', cloud: 'rgba(140,160,180,0.35)' },
    hive:        { land: ['#4a2a5a', '#5a3a6a', '#3a2048'], sea: ['#7a3a8a', '#9050a0'], ice: '#c8a0d0', cloud: 'rgba(200,160,220,0.35)' },
    shattered:   { land: ['#5a5048', '#6a6058', '#4a423c'], sea: ['#3a322c'], ice: '#9a9088', cloud: 'rgba(180,170,160,0.28)' },
  ...
  // 接收 hex 或 [r,g,b] 数组，归一为 [r,g,b]
  function toRgb(c) { return Array.isArray(c) ? c : hexRgb(c); }
  ...
  function smstep(a, b, x) { const t = M.clamp((x - a) / (b - a), 0, 1); return t * t * (3 - 2 * t); }

  function paintRocky(p) {
    const hires = p.hires === true;
    const W = hires ? 384 : 192, H = hires ? 384 : 192;
    ...
    for (let y = 0; y < H; y++) {
      for (let x = 0; x < W; x++) {
        ...
        if (p.type === 'lava') {
          ...
          const dark = hexRgb(pal.land[0]);
          const darkN = n1 * 0.3;
          const dr = dark[0] + darkN * 40, dg = dark[1] + darkN * 20, db = dark[2] + darkN * 10;
          ...
          const iceN = smstep(iceT - 0.14, iceT + 0.06, lat + (n2 - 0.5) * 0.1);
          if (iceN > 0) colArr = lerpColor(colArr, pal.ice, iceN * 0.85);
  ...
  function paintGas(p) {
    const hires = p.hires === true;
    const W = hires ? 512 : 280, H = hires ? 360 : 196;
    const c = makeCanvas(W, H);
    const ctx = c.getContext('2d');
    ...
    for (let y = 0; y < H; y++) {
      for (let x = 0; x < W; x++) {
        ...
        const ti = M.clamp(band + (n - 0.5) * 0.25, 0, 1);
        const tlen = pal.bands.length - 1;
        const col = lerpColor(pal.bands[Math.floor(ti * tlen)], pal.bands[Math.ceil(ti * tlen)], ti * tlen % 1);
        // 大红斑
        const ds = Math.hypot(u - spotX, (v - spotY) * 1.6);
        let final = col;
        if (ds < 0.14) { const t = 1 - ds / 0.14; final = lerpColor(col, pal.spot, 0.35 + t * 0.5); }
        const i = (y * W + x) * 4;
        d[i] = final[0]; d[i + 1] = final[1]; d[i + 2] = final[2]; d[i + 3] = 255;
      }
    }
    ctx.putImageData(img, 0, 0);
    return c;
