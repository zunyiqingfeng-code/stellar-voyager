...
| test/live-check.js（线上部署实测） | 对 GitHub Pages 部署版执行端到端检查：脚本加载→主菜单→零运行时异常→新游戏→飞行 HUD→存档写入 | ✔ 8/8 |
| test/perf-probe.js（性能基线） | 小行星带满屏岩石场景的 FPS / 单帧渲染耗时 / 行星贴图冷生成耗时，优化前后可对比 | ✔ FPS 60 满帧 · render ≈0.6ms |
| 无头截图 + 像素探针 | 三场景渲染验证（亮度/色相直方图/HUD 文本） | ✔ |

### v1.1.2 UI 焕新（开屏修复 × 行星美术）
- **开屏动画修复**：boot 场景之前按 canvas 裸坐标画文字，叠加引擎相机平移后跑到右下角落 —— 现在显式走屏幕空间变换，配合 130 颗动态星点 + 呼吸标题 + 进度条，开屏 1.4 秒真正好看
- **菜单行星美术**：之前是 continental + 128px 贴图放大到 380px 像马赛克 —— 换盖亚星球 + 384px 高清贴图 + 双层大气辉光（外柔内锐），告别“丑”
- **行星贴图配色 Bug**（隐藏但影响所有行星）：lerpColor 之前用 hexRgb(array) 导致 NaN 通道 → 显示为纯红色斑块 —— 现在 toRgb 正确处理 hex/数组两种入参，所有行星配色干净（红像素占比从 1.5%+ → 0%）
- **贴图分辨率提升**：rocky 128→192（游戏内）/ 384（菜单）/ gas 200×140→280×196（游戏内）/ 512×360（菜单），低配机通过 warmup 分摊无感
- **冰冠平滑化**：硬 lat 阈值改 smoothstep，过渡自然
- **雷达默认隐藏**：刷新时左下角不再出现孤立圆圈（之前 menu/boot 阶段 radar 永远可见）
- 菜单版本号 → v1.2.0

### v1.1.1 追加（雷达 × 平衡）
- **雷达小地图**：左下角圆形雷达，显示行星/空间站/敌舰/小行星带相对位置与扫描线，量程外目标钳制到边缘指示方向
- **海盗难度成长**：海盗等级随玩家科技等级共同提升（IV 级科技时海盗升级为驱逐舰舰体+反物质武器），数量上限 5；采矿伏击强度同步挂钩——后期不再无脑碾压
- 教训入库：大文件写入截断导致方法丢失的回归被 CDP 套件当场抓获并修复

### v1.1.0 优化发布（性能 × 打击感 × QoL）
**性能**
- 小行星精灵预烘焙：带内最多 90 块岩石不再每帧重建多边形路径（低配机收益最大）
- 恒星渲染缓存：光晕渐变 + 22 个米粒噪点由每帧重建改为一次性烘焙精灵
- 行星贴图异步预热：进星系后逐帧后台生成贴图，消除首次视线的微卡顿
- 星图可达性缓存：跃迁范围计算由"每帧每星系一次 BFS"改为单次缓存
- HUD 定向更新：状态条由周期性 innerHTML 重建改为仅写样式/文本，降低 GC 压力

**缺陷修复**
- 死亡重生后 `respawnT` 从不递减，导致恒星高温伤害永久失效的隐藏 bug
- `camerashake` 被引用却从未实现——现已落地为完整的屏幕震动系统

**打击感与体验**
- 屏幕震动系统：受击/爆炸/碰撞分级震感，自动衰减
- 低血量红色暗角脉冲（船体 <32% 时触发）
- 屏外敌舰红色指示箭头（1700m 内自动出现）
- 锁定目标旋转虚线环
- 接近星系边缘时提示按 M 跃迁
- 关闭页面前自动存档（含实时位置）
